...
public class TeamServiceImpl extends ServiceImpl<TeamMapper, Team>
    ...
    public boolean updateTeam(TeamUpdateRequest teamUpdateRequest, User loginUser) {
        ...
        if (!Objects.equals(loginUserId, teamUserId) && !userService.isAdmin(loginUser)) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR);
        }
        // 4.如果队伍状态改为加密，必须要有密码
    ...
    public boolean joinTeam(TeamJoinRequest teamJoinRequest, User loginUser) {
        ...
        // 只有一个线程能获取到锁
        RLock lock = redissonClient.getLock("TeamService:join_team");
        try {
            // 抢到锁并执行
            while (true) {
    ...
    public boolean deleteTeam(long id, User loginUser) {
        ...
        Long loginUserId = loginUser.getId();
        // 校验你是不是队伍的队长
        if (!Objects.equals(team.getUserId(), loginUserId)&&!userService.isAdmin(loginUser)) {
    ...
    @Override
    public List<User> getIncludeUser(long teamId) {
        // 判断队伍是否存在
        ...
        // 取用户 id
        List<Long> idList = list.stream().map(UserTeam::getUserId).collect(Collectors.toList());
