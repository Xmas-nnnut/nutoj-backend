...
//        String redisKey = "user:" + tokenId;
        redisTemplate.opsForValue().set(String.valueOf(user.getId()), user, 7, TimeUnit.DAYS);
        String token = JwtUtils.getTokenByUser(user);
        // 4. 记录用户信息脱敏
...
//        Object userObj = request.getSession().getAttribute(USER_LOGIN_STATE);
    ...
    public boolean userLogout(HttpServletRequest request) {
        if (request.getAttribute(USER_LOGIN_STATE) == null) {
